---
title: Symfony 4.4 and above overhead with opcache.preload
...
When using Symfony 4.4 and above, you are experiencing large performance impacts when `opcache.preload` is active.
...
When `newrelic.preload_framework_library_detection` and `opcache.preload` are both enabled, New Relic One queries the `opcache` status with each function call. However, when opcache preload is not successful, it can result in this query returning `null` which can cause significant overhead in New Relic's PHP agent.
